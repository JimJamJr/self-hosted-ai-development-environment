...
|Software | Role |
|---------|-------|
|Debian 12| Light weight OS|
|CasaOS | system and container management|
|Ollama |LLM hsoting|
|Open WebUI| browser-based AI interface|

The server is build on repurposed low-specification hardware configureed for reliable 24/7 operation. It is primarily used for hosting services and lightweight AI workloads. This can be reconfigured for heavier workloads with more appropriate hardware.
...
- Tailscale private networkng for encrypted device communication
...
Roo Code is integrated into the VS Code environment to enable direct interation with local and remote models via the Ollama API.
...
It provides a foundation for scalable software development while demonstrating practicle experience in:
